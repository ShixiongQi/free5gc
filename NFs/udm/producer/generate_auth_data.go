...
func GenerateAuthDataProcedure(authInfoRequest models.AuthenticationInfoRequest, supiOrSuci string) (
	...
	rand.Seed(time.Now().UnixNano()) // Generate RAND
	...
	*/
	...
	ts = time.Now().UnixNano() // Record the start timestamp of CONC generation
	...
	if authSubs.AuthenticationMethod == models.AuthMethod__5_G_AKA {
		...
		ts = time.Now().UnixNano() // Record the start timestamp of xresStar generation
		...
		P0 = []byte(authInfoRequest.ServingNetworkName)
		P1 = SQNxorAK
		ts = time.Now().UnixNano() // Record the start timestamp of Kausf derivation
